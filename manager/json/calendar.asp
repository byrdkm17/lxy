...
function json_calendar()
    ...
    if content = "" then
        ...
        if edit = "" then
            if title = "" then
                set rs = conn.execute("select * from siteinfo where type = 3 order by create_time desc")
            else
                set rs = conn.execute("select * from siteinfo where type = 3 and title like '%" & title & "%' order by create_time desc")
            end if
        else 
